...
def humanize(msg: str) -> str:
    ...
    if re.match(r"\bTouched\s+.+?\(no snippet change\)", m, re.I) or "no snippet change" in m:
        ...
        import random as _r
        # если есть путь в сообщении — попробуем сохранить его
        path_m = re.search(r"Touched\s+(.+?)\s+\(no snippet change\)", m, re.I)
        if path_m:
            path = path_m.group(1)
...
# --- monkey-patch subprocess.run to humanize commit messages ---
try:
...
# --- monkey-patch GitPython Repo.index.commit if present ---
try:
    import git as _git
...
TARGETS = [os.path.join(ROOT, p) for p in [
    "synapseatlas","synapseatlas/core","synapseatlas/graph","synapseatlas/export",
    "synapseatlas/plugins","synapseatlas/utils","synapseatlas/tasks","synapseatlas/bookmarks"]]
